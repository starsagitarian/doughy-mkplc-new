              ...
              nearby.map((bakery) => {
                return (
                  ...
                  >
                    <div className='ekrUEE'>
                      <div className='Single-Bakery-Card'>
                        <div className='bakery-card-img'></div>
                        <div className='bakery-card-info'>
                          <li key={bakery.id} className='bakery-list'>
                            <h1>{bakery.name}</h1>
                            <h5>{bakery.Description} </h5>
    ...
    <>
      ...
      <div className='show-map-btns'>
        <div className='map-btn'>
          <Button onClick={() => handleMap()} size='small' color='primary'>
            map
          </Button>
          <div className={classes.root}></div>
        </div>
        <div className='list-btn'>
          <Button onClick={() => handleList()} size='small' color='primary'>
            list
          </Button>
        </div>
      </div>
      {isForDelivery ? (
        <div className='list-head'>
          {nearby.length ? (
            <h2> There are {nearby.length} Bakeries Available for Delivery</h2>
          ) : (
            <p>Nothing available for Delivery</p>
          )}{" "}
        </div>
        ...
        <div className='list-head'>
          {db.length ? (
            <h2> There are {db.length} Bakeries Available for Pickup</h2>
          ) : (
            <p>Nothing available for Pickup</p>
          )}{" "}
        </div>
      )}
      {isForDelivery ? (
        <div>{isClicked ? map : notMapDelivery}</div>
      ) : (
        <div>{isClicked ? map : notMapPickup}</div>
      )}
    </>
  );
};
